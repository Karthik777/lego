...
Only the bare `/` is rewritten, so `/static`, `/muhurtha/feed.ics` and every other path still resolve normally on both hostnames — which matters more than it used to, because a calendar client subscribes to a deep path and a greedy rewrite would break it. Set `APEX_ROUTE=/hora` to put the apex back on the hora page, and `HORA_DOMAIN` to move the apex itself — it feeds both the Caddyfile and the block's canonical and `og:` URLs.
