...
def create_dummy_chat_data():
    ...
    proj.insert_all([dict(name=n,description=d,created_at=time(),user_id=uid) for n,d in ps])
    mk_time = lambda md=0: mktime((datetime.now() - timedelta(days=(30 * md) + randint(0, 3))).timetuple())
    ...
    def _(md=0):
        ...
        chat = chats.insert(dict(id=uuid4(), user_id=uid, name=f'Test Chat {uuid4()}', shared_with=json.dumps([shared_uid]),
            project_id=pid, created_at=ts, updated_at=ts + 10))

        docs.insert(dict(id=doc1, user_id=uid, chat_id=chat.id, project_id=pid, name="foo.pdf", mime_type="application/pdf",
                         storage_url=f"{uid}/docs/foo.pdf", created_at=ts))
        docs.insert(dict(id=doc2, user_id=uid, chat_id=chat.id, project_id=pid, name="bar.txt", mime_type="text/plain",
                         storage_url=f"{uid}/docs/bar.txt", created_at=ts + 1))

        msgs.insert(dict(chat_id=chat.id, sender="user", message="Summarise foo.pdf", document_ids=json.dumps([doc1]),created_at=ts + 2))
        msgs.insert(dict(chat_id=chat.id, sender="assistant", message="Here's a summary of foo.pdf.", document_ids="",created_at=ts + 3))
        msgs.insert(dict(chat_id=chat.id, sender="user", message="Also consider bar.txt", document_ids=json.dumps([doc2]),created_at=ts + 4))

        ckpts.insert(dict(chat_id=chat.id, user_id=uid, name="Initial Summary", last_msg_id=2, created_at=ts + 5))
