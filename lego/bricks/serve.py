"Every route the brick package serves. Nothing here imports the application it is mounted in."

from starlette.responses import HTMLResponse, JSONResponse, PlainTextResponse

from .bricks import find, manifest
...
def connect(app, page=None):
    "Register the brick routes. `page` wraps the gallery in the host's shell; without one it is bare."
    shell = page or (lambda content, **kw: HTMLResponse(ui.bare(content)))

    @app.get(Routes.index)
    def bricks_index(req, auth=None): return shell(ui.gallery(), title='Bricks')
